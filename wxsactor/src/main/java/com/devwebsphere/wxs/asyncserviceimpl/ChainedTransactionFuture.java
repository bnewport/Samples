 ...
 * @param <T>
 */
public class ChainedTransactionFuture<K extends Serializable> implements java.util.concurrent.Future<Map<K, KeyOperatorResult<K>>>,
		SerializedFuture<Map<K, KeyOperatorResult<K>>> {
	 ...
	 */
	public Map<K, KeyOperatorResult<K>> peekResult() throws ObjectGridException {
		...
		try {
			...
			while (isSendable) {
				try {
					...
					String msg = "Exception during peek result for " + id;
	 ...
	 */
	public Map<K, KeyOperatorResult<K>> get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
		return get();
	}
		...
		try {
			...
			while (isSendable) {
				try {
					...
					String msg = "Exception during isDone for " + id;
					logger.log(Level.SEVERE, msg, e);
					isSendable = WXSUtils.isRetryable(e);
					if (!isSendable) {
						throw new ObjectGridRuntimeException(msg, e);
			...
			logger.log(Level.SEVERE, "Exception during isDone result for " + id, e);
