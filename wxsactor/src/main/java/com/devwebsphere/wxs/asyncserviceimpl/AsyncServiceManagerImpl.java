...
public class AsyncServiceManagerImpl implements AsyncServiceManager
{
		...
		throws ObjectGridException
	{
		...
		{
			ObjectGridManager m = ObjectGridManagerFactory.getObjectGridManager();
			ClientClusterContext ccc = m.connect(null, null);
			clientGrid = m.getObjectGrid(ccc, grid.getName());
		}
 	}
	 ...
	 * @throws ObjectGridException
	 */
	public <V> AsyncServiceFuture<V> sendAsyncMessage2(Session userSession, Serializable message, RoutableKey id)
		throws ObjectGridException
	{
		Session sess = null;
		if(userSession != null)
	...
	public <V> AsyncServiceFuture<V> sendAsyncMessage(Serializable message)
		throws ObjectGridRuntimeException
	{
		try
		{
			...
			int pid = id.hashCode() % map.getPartitionManager().getNumOfPartitions();
	...
	public <R> AsyncServiceFuture<R> scheduleDurableLocalJob(Session sess, Job<R> job)
		throws ObjectGridRuntimeException
	{
		try
		{
	...
	public <K extends Serializable, D extends KeyOperator<K>> Future<Map<K, KeyOperatorResult<K>>> doChainedTransaction(
			Session session,
			String mapName,
			Map<K, D> operators) 
	{
