...
public class TestSubstringIndex {
	...
	@BeforeClass
	static public void initGrid() throws IOException {
	...
	@Test
	public void testGeneratePrefix() throws IllegalAccessException, NoSuchFieldException {
		PrefixIndex p = TestBusinessObject.class.getDeclaredField("firstName").getAnnotation(PrefixIndex.class);
	...
	static public void preloadGrid2() throws IOException {
	...
	static public void preloadGrid() throws IOException {
	...
	public void multipleSearch() {
		...
		criteria.surname = "RAG";
		SearchResult<Long> matches = indexManager.searchMultipleIndexes(criteria, true);
		Assert.assertFalse(matches.isTooManyMatches());
		Assert.assertEquals(1, matches.getResults().size());

		matches = indexManager.searchMultipleIndexes(criteria, false);
		Assert.assertFalse(matches.isTooManyMatches());
		Assert.assertEquals(2, matches.getResults().size());
	}
