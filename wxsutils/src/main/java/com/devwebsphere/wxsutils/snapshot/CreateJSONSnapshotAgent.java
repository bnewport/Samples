...
public class CreateJSONSnapshotAgent implements ReduceGridAgent {
	...
	/**
	 * This calls the method findAllKeys with either no arguments or one argument depending on whats present.
	 * 
	 * @param mki
	 * @return
	 */
	...
	static public Iterator<Object> getAllKeys(ObjectMap map) {
		try {
			...
			MapIndex index = (MapIndex) map.getIndex(MapIndexPlugin.SYSTEM_KEY_INDEX_NAME);
	...
	/**
	 * This writes a remote disk based snapshot of a given map in a grid. The snapshot files are placed remotely in the
	 * specified rootFolder
	 * 
	 * @param utils
	 *            The client to the grid with the map
	 * @param mapName
	 *            The map to dump
	 * @param rootFolder
	 *            The name of a directory. It will be created if it is missing
	 * @throws ObjectGridRuntimeException
	 */
	public static void writeSnapshot(WXSUtils utils, String mapName, String rootFolder) throws ObjectGridRuntimeException {
		try {
