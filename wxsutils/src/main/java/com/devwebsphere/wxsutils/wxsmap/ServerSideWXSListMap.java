...
/**
 * This is a WXSMapOfLists implementation for use on the server side typically in an Agent. This
 * takes the local session provided to the agent and the name of the list and then allows the various
 * list operations on that list. This all happens using the transaction of the 'agent' or the session
 * provided to this class. This is a slightly different behavior than on the client where
 * each method call is its own transaction. This is NOT the case on the server side. The provided
 * sessions transaction scopes all method calls.
 * @author bnewport
 *
 * @param <K>
 * @param <V>
...
public class ServerSideWXSListMap<K extends Serializable, V extends Serializable> implements WXSMapOfLists<K, V> 
{
	...
	public void lpush(K key, List<V> values) {
		BigListPushAgent.push(sess, map, LR.LEFT, Collections.singletonList(key), Collections.singletonList(WXSMapOfBigListsImpl.convertToBulkList(values)), null);
	}

	public void lpush(K key, List<V> values, K dirtySet) {
		BigListPushAgent.push(sess, map, LR.LEFT, Collections.singletonList(key), Collections.singletonList(WXSMapOfBigListsImpl.convertToBulkList(values)), dirtySet);
	}

	public void lpush(
			Map<K, List<BulkPushItem<V>>> items) 
	{
		ArrayList<K> keys = new ArrayList(items.keySet());
	...
	{
		return BigListPopNItemsAgent.popNItems(sess, map, key, LR.LEFT, numItems, dirtyKey);
	}

	public List<V> lpop(K key, int numItems) {
		return BigListPopNItemsAgent.popNItems(sess, map, key, LR.LEFT, numItems, null);
	}

	public List<V> rpop(K key, int numItems, K dirtyKey) {
		return BigListPopNItemsAgent.popNItems(sess, map, key, LR.RIGHT, numItems, dirtyKey);
	}

	public List<V> rpop(K key, int numItems) {
		return BigListPopNItemsAgent.popNItems(sess, map, key, LR.RIGHT, numItems, null);
	}

	public int rremove(K key, int numItems) 
	{
		return BigListRemoveNItemsAgent.removeNItems(sess, map, key, LR.RIGHT, numItems, null, false);
	}
	...
	public int lremove(K key, int numItems) {
		return BigListRemoveNItemsAgent.removeNItems(sess, map, key, LR.LEFT, numItems, null, false);
	}
	...
	{
		BigListPushAgent.push(sess, map, LR.RIGHT, Collections.singletonList(key), Collections.singletonList(WXSMapOfBigListsImpl.convertToBulkList(values)), null);
	}

	public void rpush(K key, List<V> values, K dirtySet) {
		BigListPushAgent.push(sess, map, LR.RIGHT, Collections.singletonList(key), Collections.singletonList(WXSMapOfBigListsImpl.convertToBulkList(values)), dirtySet);
	}
