...
/**
 * BigLists are split in buckets of at most BUCKET_SIZE items. Items can be
 * added to the left or right of the list using the push operation and popped
 * from either side also. The list is stored in two maps. One has meta data with
 * the number of the left and right bucket. The other holds the buckets as
 * normal lists. The leftmost and rightmost buckets can have less than
 * BUCKET_SIZE items but buckets in the middle are always full. Items can only
 * be removed from the left or right using pop.
 * 
 * Eviction Design. All list eviction times are scheduled for the minute
 * following the actual time. A scheduled list has its key added to a set named
 * after the list. A list keeps track of eviction sets and pops them when they
 * are removed. A well known list tracks lists within this shard with eviction
 * enabled
 * 
 * @author bnewport
 * 
...
public class BigListHead<V extends Serializable> implements Serializable {
	...
	void updateEviction(Session sess, ObjectMap map, Object key) {
		...
		default:
			...
			throw new ObjectGridRuntimeException("Unknown eviction type:"
					+ evictionType);
		}
	}
	...
	public <K extends Serializable> void setEvictionPolicy(Session sess,
			ObjectMap map, Object key, EvictionType eType, int intervalSeconds) {
		try {
			String listName = getListNameFromHeadMapName(map.getName());
			String evictSetName = WXSMapOfBigListsImpl
					.getListEvictionSetMapName(listName); // Long,
															// Set<K>
			String evictListName = WXSMapOfBigListsImpl
					.getListEvictionListMapName(listName);
			...
			if (SetIsEmptyAgent.isEmpty(sess, eSetMap, evictSetKey)) {
				...
				l.add(evictSetKey);
				BulkPushItem<Long> bpa = new BulkPushItem<Long>(evictSetKey,
						(Filter) null);
				List<BulkPushItem<Long>> value = (List<BulkPushItem<Long>>) Collections
						.singletonList(bpa);
				// most recent eviction set on left, oldest or next to expire on
				// right
				BigListPushAgent.push(sess, eList, LR.LEFT, keyList,
						Collections.singletonList(value), null);
			}
			// add list to set for this time
			SetAddRemoveAgent.doOperation(sess, eSetMap, evictSetKey,
					Operation.ADD, (K) key);
		} catch (Exception e) {
	...
	/**
	 * Given the name of the head template map for a list, this returns the list
	 * name
	 * 
	 * @param lhMapName
	 * @return
	...
	/**
	 * This returns the name of the bucket map for this list
	 * 
	 * @param sess
	 * @param map
	 *            The head map
	...
	ObjectMap getBucketMap(Session sess, ObjectMap map) {
		String listName = getListNameFromHeadMapName(map.getName());
		String bucketMapName = WXSMapOfBigListsImpl
				.getListBucketMapName(listName);
		try {
			return sess.getMap(bucketMapName);
		} catch (UndefinedMapException e) {
			logger.log(Level.SEVERE, "Unknown map " + bucketMapName, e);
			throw new ObjectGridRuntimeException("Bucket map doesn't exist "
					+ bucketMapName, e);
		}
	}
	...
	/**
	 * This returns the key for a bucket of list elements given the bucket
	 * number
	 * 
	 * @param key
	 * @param bucket
	...
	public BigListHead(Session sess, ObjectMap map, Object key, V value,
			int bSize) throws ObjectGridException {
	 ...
	 */
	public void push(Session sess, ObjectMap map, Object key, LR isLeft,
			V value, Filter cfilter) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
		// if a filter is specified then if any matching
		// elements exist, do not push.
		if (cfilter != null) {
			ArrayList<V> matchingElements = range(sess, map, key, 0,
					Integer.MAX_VALUE, cfilter);
			if (matchingElements.size() > 0)
				return;
		}
		while (true) {
			// get current bucket
			String bkey = getBucketKey(key, isLeft == LR.LEFT ? leftBucket
					: rightBucket);
	...
	public <K extends Serializable> List<V> popNItems(Session sess,
			ObjectMap map, Object key, LR isLeft, int n, K dirtyKey,
			boolean releaseLease) throws ObjectGridException {
	...
	public <K extends Serializable> int removeNItems(Session sess,
			ObjectMap map, Object key, LR isLeft, int n, K dirtyKey,
			boolean releaseLease) throws ObjectGridException {
		int counter = 0;
		for (int i = 0; i < n; ++i) {
	...
	public <K extends Serializable> V pop(Session sess, ObjectMap map,
			Object key, LR isLeft, K dirtyKey) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
		String bkey = getBucketKey(key, isLeft == LR.LEFT ? leftBucket
				: rightBucket);
	 ...
	 * @throws ObjectGridException
	 */
	<K extends Serializable> void removeFromDirtySet(Session sess,
			ObjectMap map, Object key, K dirtyKey) throws ObjectGridException {
		if (dirtyKey != null) {
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, "Removing key [" + key
						+ "] from dirtySet [" + dirtyKey + "]");
			}
			ObjectMap dirtyMap = sess.getMap(BigListPushAgent
					.getDirtySetMapNameForListMap(map.getName()));
			SetAddRemoveAgent.doOperation(sess, dirtyMap, dirtyKey,
					Operation.REMOVE, (Serializable) key);
			ObjectMap leaseMap = sess.getMap(BigListPushAgent
					.getDirtySetLockMapNameForListMap(map.getName()));
			// remove the lock on this key if present
			if (leaseMap.containsKey(key)) {
				if (logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINE, "List empty, cleaning up lease for "
							+ key);
				}
				leaseMap.remove(key);
			}
	...
	void releaseLease(Session sess, ObjectMap map, Object key)
		...
		if (leaseMap.containsKey(key)) {
			if (logger.isLoggable(Level.FINE))
				logger.log(Level.FINE, "Removing lease for " + key
						+ " in grid " + sess.getObjectGrid().toString());
			leaseMap.remove(key);
		} else {
			if (logger.isLoggable(Level.FINE))
				logger.log(Level.FINE, "No lease found for " + key
						+ " in grid " + sess.getObjectGrid().toString());
		}
	}
	...
	public int size(Session sess, ObjectMap map, Object key)
		...
		if (leftBucket == rightBucket) {
			ArrayList<V> list = (ArrayList<V>) bmap.get(getBucketKey(key,
					leftBucket));
			return list.size();
		} else {
			ArrayList<V> lList = (ArrayList<V>) bmap.get(getBucketKey(key,
					leftBucket));
			ArrayList<V> rList = (ArrayList<V>) bmap.get(getBucketKey(key,
					rightBucket));
	...
	public <K extends Serializable> ArrayList<V> popAll(Session sess,
			ObjectMap map, Object key, K dirtyKey) throws ObjectGridException {
		ArrayList<V> list = new ArrayList<V>();
		ObjectMap bmap = getBucketMap(sess, map);
		for (int i = leftBucket; i <= rightBucket; ++i) {
			ArrayList<V> blist = (ArrayList<V>) bmap
					.remove(getBucketKey(key, i));
			list.addAll(blist);
		}
		map.remove(key);
		if (dirtyKey != null)
	...
	public ArrayList<V> range(Session sess, ObjectMap map, Object key, int low,
			int high, Filter filter) throws ObjectGridException {
		int size = size(sess, map, key);
		try {
			if (low < 0 || high < 0 || high < low)
				throw new ObjectGridRuntimeException(
						"Low and High are invalid (<0 || high < low");
			ObjectMap bmap = getBucketMap(sess, map);
			if (size > 0 && low < size) {
				if (high >= size)
					high = size - 1;
				...
				ArrayList<V> current = (ArrayList<V>) bmap.get(getBucketKey(
						key, bucket));
				for (int i = low; i <= high; i++) {
					...
					if (offset >= current.size() && i != high) {
						bucket++;
						offset = 0;
						current = (ArrayList<V>) bmap.get(getBucketKey(key,
								bucket));
					}
				}
				return rc;
			...
			logger.log(Level.SEVERE,
					"Range Exception for key " + key.toString() + ",Low:" + low
							+ ",High:" + high + ",Size:" + size, e);
			throw new ObjectGridRuntimeException(e);
		}
	}
	...
	public void trim(Session sess, ObjectMap map, Object key, int size)
		...
		try {
			...
			if (size == 0)
				...
				ObjectMap bmap = getBucketMap(sess, map);
				ArrayList<V> lList = (ArrayList<V>) bmap
						.getForUpdate(getBucketKey(key, leftBucket)); // BN
