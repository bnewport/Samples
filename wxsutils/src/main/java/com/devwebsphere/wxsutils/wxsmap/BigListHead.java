...
/**
 * BigLists are split in buckets of at most BUCKET_SIZE items. Items can be added to the left or right of the list using
 * the push operation and popped from either side also. The list is stored in two maps. One has meta data with the
 * number of the left and right bucket. The other holds the buckets as normal lists. The leftmost and rightmost buckets
 * can have less than BUCKET_SIZE items but buckets in the middle are always full. Items can only be removed from the
 * left or right using pop.
 * 
 * Eviction Design. All list eviction times are scheduled for the minute following the actual time. A scheduled list has
 * its key added to a set named after the list. A list keeps track of eviction sets and pops them when they are removed.
 * A well known list tracks lists within this shard with eviction enabled
 * 
 * @author bnewport
 * 
 */
public class BigListHead<V extends Serializable> implements Serializable {
	...
	public <K extends Serializable> void setEvictionPolicy(Session sess, ObjectMap map, Object key, EvictionType eType, int intervalSeconds) {
		try {
			...
			String evictSetName = WXSMapOfBigListsImpl.getListEvictionSetMapName(listName); // Long,
																							// Set<K>
	...
	/**
	 * Given the name of the head template map for a list, this returns the list name
	 * 
	 * @param lhMapName
	 * @return
	 */
	...
	/**
	 * This returns the key for a bucket of list elements given the bucket number
	 * 
	 * @param key
	 * @param bucket
	 * @return
	...
	public BigListHead(Session sess, ObjectMap map, Object key, V value, int bSize) throws ObjectGridException {
	 ...
	 */
	public void push(Session sess, ObjectMap map, Object key, LR isLeft, V value, Filter cfilter) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
	...
	public <K extends Serializable> List<V> popNItems(Session sess, ObjectMap map, Object key, LR isLeft, int n, K dirtyKey, RELEASE releaseLease)
			throws ObjectGridException {
		ArrayList<V> list = new ArrayList<V>(n);

		for (int i = 0; i < n; ++i) {
			V rc = pop(sess, map, key, isLeft, dirtyKey, releaseLease == RELEASE.WHEN_EMPTY);
			if (rc != null)
				list.add(rc);
			else
				break;
		...
		// handle ALWAYS, WHEN_EMPTY is handled above in pop()
		if (dirtyKey != null && releaseLease == RELEASE.ALWAYS) {
			releaseLease(sess, map, key);
		}
		return list;
	}

	public <K extends Serializable> int removeNItems(Session sess, ObjectMap map, Object key, LR isLeft, int n, K dirtyKey, RELEASE releaseLease)
			throws ObjectGridException {
		int counter = 0;
		for (int i = 0; i < n; ++i) {
			V rc = pop(sess, map, key, isLeft, dirtyKey, releaseLease == RELEASE.WHEN_EMPTY);
	...
	<K extends Serializable> V pop(Session sess, ObjectMap map, Object key, LR isLeft, K dirtyKey, boolean releaseWhenEmpty)
			throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
		String bkey = getBucketKey(key, isLeft == LR.LEFT ? leftBucket : rightBucket);
		...
		if (list != null) {
			...
			if (list.isEmpty()) // a bucket is now empty
			{
				...
				if (leftBucket == rightBucket) // the list is now empty
				{
					map.remove(key);
					if (dirtyKey != null) {
						removeFromDirtySet(sess, map, key, dirtyKey, releaseWhenEmpty);
					}
				} else {
					if (isLeft == LR.LEFT)
						leftBucket++;
	 ...
	 * @throws ObjectGridException
	 */
	<K extends Serializable> void removeFromDirtySet(Session sess, ObjectMap map, Object key, K dirtyKey, boolean releaseLease)
			throws ObjectGridException {
		if (dirtyKey != null) {
	...
	void releaseLease(Session sess, ObjectMap map, Object key) throws UndefinedMapException, ObjectGridException {
		ObjectMap leaseMap = sess.getMap(BigListPushAgent.getDirtySetLockMapNameForListMap(map.getName()));
		if (leaseMap.remove(key) != null) {
			if (logger.isLoggable(Level.FINE))
	...
	public int size(Session sess, ObjectMap map, Object key) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
	...
	public void remove(Session sess, ObjectMap map, Object key) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
	...
	public <K extends Serializable> ArrayList<V> popAll(Session sess, ObjectMap map, Object key, K dirtyKey) throws ObjectGridException {
		...
		map.remove(key);
		if (dirtyKey != null)
			removeFromDirtySet(sess, map, key, dirtyKey, true);
		return list;
	}

	public ArrayList<V> range(Session sess, ObjectMap map, Object key, int low, int high, Filter filter) throws ObjectGridException {
	...
	public void trim(Session sess, ObjectMap map, Object key, int size) throws ObjectGridException {
		try {
			if (size < 0)
