...
/**
 * BigLists are split in buckets of at most BUCKET_SIZE items. Items can be added to the left or
 * right of the list using the push operation and popped from either side also. The list is stored
 * in two maps. One has meta data with the number of the left and right bucket. The other holds the
 * buckets as normal lists. The leftmost and rightmost buckets can have less than BUCKET_SIZE items
 * but buckets in the middle are always full. Items can only be removed from the left or right using
 * pop.
 * 
 * Eviction Design. All list eviction times are scheduled for the minute following the actual time.
 * A scheduled list has its key added to a set named after the list. A list keeps track of eviction
 * sets and pops them when they are removed. A well known list tracks lists within this shard with
 * eviction enabled
 * 
 * @author bnewport
 * 
...
public class BigListHead<V extends Serializable> implements Serializable {
	...
	public <K extends Serializable> V pop(Session sess, ObjectMap map, Object key, LR isLeft, K dirtyKey) throws ObjectGridException {
		ObjectMap bmap = getBucketMap(sess, map);
		String bkey = getBucketKey(key, isLeft == LR.LEFT ? leftBucket : rightBucket);
	 ...
	 * @throws ObjectGridException
	 */
	<K extends Serializable> void removeFromDirtySet(Session sess, ObjectMap map, Object key, K dirtyKey) throws ObjectGridException {
		if (dirtyKey != null) {
			...
			if (leaseMap.containsKey(key)) {
				if (logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINE, "List empty, cleaning up lease for " + key);
				}
				leaseMap.remove(key);
			}
		}
